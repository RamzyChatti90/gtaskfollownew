package com.gtaskfollownew.repository;
...
public interface CommentRepository extends JpaRepository<Comment, Long> {
    ...
    @Query(
        value = "select comment from Comment comment left join fetch comment.task left join fetch comment.author",
        countQuery = "select count(comment) from Comment comment"
    )
    Page<Comment> findAllWithToOneRelationships(Pageable pageable);

    @Query("select comment from Comment comment left join fetch comment.task left join fetch comment.author")
    List<Comment> findAllWithToOneRelationships();

    @Query("select comment from Comment comment left join fetch comment.task left join fetch comment.author where comment.id =:id")
