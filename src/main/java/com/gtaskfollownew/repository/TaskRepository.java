...
public interface TaskRepository extends JpaRepository<Task, Long> {
    ...
    @Query(
        value = "select task from Task task left join fetch task.assignee left join fetch task.category",
        countQuery = "select count(task) from Task task"
    )
    Page<Task> findAllWithToOneRelationships(Pageable pageable);

    @Query("select task from Task task left join fetch task.assignee left join fetch task.category")
    List<Task> findAllWithToOneRelationships();

    @Query("select task from Task task left join fetch task.assignee left join fetch task.category where task.id =:id")
